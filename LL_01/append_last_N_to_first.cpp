...
Node *appendLastNToFirst(Node *head, int n)
{
    //Write your code here
    Node *temp = head;
    int size = length(head); // funcn call -> to get the length of the main LL.
    int diff = size - n;   //diff must be positive. if it is not then it means that n > LL length. In that case No operation is going to be performed.
    ...
    if(diff > 0 && n > 0) {
        ...
        temp -> next = NULL; // After this line our main LL gets divided into two Smaller LL.

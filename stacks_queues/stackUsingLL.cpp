...
class Stack { 
   ...
   public:
    Stack() {
        ...
        head = NULL;
        size = 0; // indicates that stack is empty or we can say no node is present.
    }
    ...
    void push(int element) {
        ...
        size++; // everytime we will create a new node we have to increment the size by 1.
    ...
    int pop() {
        ...
        if(head == NULL) // means stack is empty. 
        {
        ...
        Node *nxt = head -> next;
        delete head; // head is now the dangling pointer.
        head = nxt;
        
        size--; //since we poped out the data from the stack.hence, size reduced by 1.

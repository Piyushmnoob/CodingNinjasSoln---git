...
bool areIdentical(TreeNode<int> *root1, TreeNode<int> * root2) {
   ...
   bool ans = true; // ans = true because for the root -> data till now root1 & root2 r identical.
    
    for(int i = 0; i < root1 -> children.size(); i++) {
        bool smallAns = areIdentical(root1 -> children[i], root2 -> children[i]); // recursive call.

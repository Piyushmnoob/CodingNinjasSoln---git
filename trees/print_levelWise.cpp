...
void printLevelWise(TreeNode<int>* root) {
    ...
    queue<TreeNode<int> *> printPendingNodes;
    
    printPendingNodes.push(root);
    
    while(printPendingNodes.size() != 0) {
        
        TreeNode<int> *front = printPendingNodes.front();
        ...
        for(int i = 0; i < front -> children.size(); i++) {
            ...
            if(i == (front -> children.size() - 1)) {
              ...
              printPendingNodes.push(front -> children[i]); // pushing the child node into queue  
